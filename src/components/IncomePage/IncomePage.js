import IncomeData from "../../utils/dummy_incomes_unique.json";
...
const IncomePage = () => {
  // Summarize the incomes by category
  const summarizedIncomes = IncomeData.reduce((accumulator, currentValue) => {
...
// Convert the summary into the data format expected by PieChart
  const pieChartData = Object.keys(summarizedIncomes).map(
    (category, index) => ({
      id: index,
      value: summarizedIncomes[category],
      label: category,
    })
  );
  return (
    <div className="flex flex-col sm:flex-row flex-1">
      ...
      <aside className="p-6 m-2 flex-[1_1_100px] border-2 border-black rounded-badge ">
        <h2 className="text-3xl font-[Poppins-bold] p-5">Income Chart</h2>
        <PieChart
          colors={[
            "red",
            "blue",
